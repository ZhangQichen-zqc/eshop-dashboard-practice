...
### 15.1 决策摘要
- [x] **530.** 在 `subprojects/decision_board.py` 计算经营健康度评分
- [x] **531.** 识别 TOP 3 增长机会
- [x] **532.** 识别 TOP 3 经营风险
- [x] **533.** 生成推荐动作清单（P0/P1/P2 优先级）

### 15.2 证据链
- [x] **534.** 每条建议标注来源模块
- [x] **535.** 每条建议标注置信度和适用条件
- [x] **536.** 每条建议标注已知风险

### 15.3 动作管理
- [x] **537.** 实现运营动作创建（CRUD）
- [x] **538.** 动作绑定用户/商品/活动
- [x] **539.** 动作写入 admin_action_logs
- [x] **540.** 动作状态跟踪
...
### 15.5 R11 API 与前端
- [x] **545.** 添加 `GET /api/r11/summary`、`/opportunities`、`/risks`、`/actions`
- [x] **546.** 添加 `POST /api/r11/actions`、`PUT /api/r11/actions/{id}`
- [x] **547.** 实现经营健康度仪表盘
- [x] **548.** 实现三栏布局（机会|风险|动作）
- [x] **549.** 实现动作管理面板和效果追踪
- [x] **550.** 实现"生成经营周报"按钮
- [x] **551.** 提交：R11 综合决策中心完成
...
### 16.1 页面布局
- [x] **552.** 创建 `analytics_dashboard/static/index.html` 完整首页
- [x] **553.** 实现左侧导航栏（可折叠，9 个一级菜单）
- [x] **554.** 实现顶部状态栏（项目名+数据状态+刷新+时间）
- [x] **555.** 实现面包屑导航
- [x] **556.** 实现前端路由（SPA）

### 16.2 通用组件
- [x] **557.** 实现可复用 KPI 卡片组件
- [x] **558.** 实现可复用数据表格组件（排序/分页/行选择）
- [x] **559.** 实现可复用筛选器栏组件
- [x] **560.** 实现可复用图表容器组件（标题+图表+口径说明）
- [x] **561.** 实现 Loading 骨架屏和空状态组件
- [x] **562.** 实现 Toast 消息通知组件
- [x] **563.** 实现 Modal 弹窗组件
- [x] **564.** 实现 CSV 导出通用按钮

### 16.3 口径说明
- [x] **565.** 每个 KPI 卡片增加"口径"按钮
- [x] **566.** 点击弹窗展示指标定义/公式/来源表/过滤条件
- [x] **567.** 确保所有页面的指标都有口径追溯

### 16.4 首页总览
- [x] **568.** 聚合 R1-R11 所有关键发现
- [x] **569.** 展示本周核心 KPI + 趋势 + TOP 3 告警 + 快捷导航
- [x] **570.** 提交：仪表盘前端整体完善
